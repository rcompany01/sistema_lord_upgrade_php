...
<?php
	...
	$class->BuscaOrcamentoID($_GET['PreviaOrc']);
	$row = mysqli_fetch_assoc($class->BuscaOrcamentoID);
...
<div class="cx-previa-orc">
	...
	<div class="cx-dados-previa">
		...
		<div class="caixas-dados-previa">
			<h4 class="nome-campos-previa"><b>Solicitante: </b><?= $row['solicitante_orc'] ?></h4>
			<h4 class="nome-campos-previa"><b>Endereço: </b><?= $row['rua_orc'] ?></h4>
			<h4 class="nome-campos-previa"><b>Evento: </b><?= $class->NomeSetor($row['evento_orc']) ?></h4>
			<h4 class="nome-campos-previa"><b>Responsável: </b><?= $row['responsavel'] ?></h4>
		</div>

		<div class="caixas-dados-previa">
			<h4 class="nome-campos-previa"><b>Total de Prestadores: </b><?= $qtdPrest ?></h4>
			<h4 class="nome-campos-previa"><b>Data de Solicitação: </b><?= $class->FormataData($row['data_sol']) ?></h4>
...
<?php
	...
	while($esc = mysqli_fetch_assoc($class->RelacaoEscalaPrest)){
		...
		$totalValor += $esc['vl_faturamento'];
?>
		<tr>
			<td><?= $esc['id_prestador'] ?></td>
			<td><?= $prest->NomePrestador($esc['id_prestador']) ?></td>
			<td><?= $func->NomeFuncao($esc['id_funcao']) ?></td>
			<td><?= $horas." HRS" ?></td>
			<td><?= $esc['entrada'] ?></td>
			<td><?= $esc['saida'] ?></td>
			<td><?= "R$ ".number_format($esc['vl_faturamento'],2) ?></td>
		</tr>
<?php
	}
?>
